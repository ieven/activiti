...
@Component
@Path("/yingfu")
public class YingfuControler extends AbstractControler {

    @Autowired
    private YingfuService service;
    ...
    public Response getReceiveList(String json) {
        ...
        try {
            ...
            List libs = service.getPaymentList(input);
            String records = service.getPaymentCount(input);
    ...
    public Response addProLib(String json) {
        ...
        try {
            Map<String, String> input = JSONHelper.toObject(json, Map.class);
            String id = service.addPayment(input) + "";
            input.put("id", id);
