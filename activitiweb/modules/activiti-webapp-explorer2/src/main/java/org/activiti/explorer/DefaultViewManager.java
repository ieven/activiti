...
public class DefaultViewManager implements ViewManager, Serializable {
    ...
    public void showTaskPage(String taskId) {
        ...
        if (task == null) {
            ...
            boolean isOwner = historyService.createHistoricTaskInstanceQuery().taskId(taskId).taskOwner(loggedInUserId)
                    .count() == 1;
            if (isOwner) {
    ...
    protected void showNavigationError(String taskId) {
        ...
        ExplorerApp.get().getNotificationManager().showErrorNotification(Messages.NAVIGATION_ERROR_NOT_INVOLVED_TITLE,
